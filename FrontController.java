    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!error.isEmpty()) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new IllegalArgumentException("ETU002720 le paramètre " + parameter.getName() + " de la méthode " + method.getName() + " n'est pas annoté avec @RequestBody ou @GetParam");
            }
        }
        out.close();
    }
    ...
    private Object[] getMethodParameters(Method method, HttpServletRequest request) throws Exception {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (parameter.isAnnotationPresent(RequestBody.class)) {
                ...
                parameterValues[i] = paramMap.get(param.value()); // Assuming all parameters are strings for simplicity
            } else {
                throw new IllegalArgumentException("ETU002720 le paramètre " + parameter.getName() + " de la méthode " + method.getName() + " n'est pas annoté avec @RequestBody ou @GetParam");
            }
        }
